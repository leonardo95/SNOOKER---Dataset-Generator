...
class DatasetGenerator:
    ...
    def build_datasets(self, countries_path):
        ...
        Utils.debug_and_log_data(True, self.logger, f'Memory spent by interface and imports: {psutil.Process().memory_info().rss / (1024 * 1024)} MB\n###### GENERATION DEBUGGING ######\n')
        ...
        ticket_generator = TicketGenerator(self.gen_id, self.domain, self.generation_params, self.logger)
        ...
        if not self.canceled:
            ...
            wait_time, curr_time = Utils.get_function_time_spent(curr_time)
            Utils.debug_and_log_data(True, self.logger, f'Analyst Train Assignment Time spent: {wait_time} seconds\nTrain Tickets assignment memory: {psutil.Process().memory_info().rss / (1024 * 1024)} MB')
        ...
        if not self.canceled:
            ...
            wait_time, curr_time = Utils.get_function_time_spent(curr_time)
            Utils.debug_and_log_data(True, self.logger, f'Train Output Time spent: {wait_time} seconds\nOutput Train Tickets memory: {psutil.Process().memory_info().rss / (1024 * 1024)} MB')
        ...
        if not self.canceled:
            ...
            wait_time, curr_time = Utils.get_function_time_spent(curr_time)
            Utils.debug_and_log_data(True, self.logger, f'Unsolved Test Output Time spent: {wait_time} seconds\nUnsolved Output Test Tickets memory: {psutil.Process().memory_info().rss / (1024 * 1024)} MB')

            wait_time, curr_time = Utils.get_function_time_spent(curr_time)
            Utils.debug_and_log_data(True, self.logger, f'Test Output Time spent: {wait_time} seconds\nOutput Complete Test Tickets memory: {psutil.Process().memory_info().rss / (1024 * 1024)} MB')
