...
class AnalystEmulation:
        ...
        if analysts_in_shift:
            ...
            if analysts_free:
                ...
                if analysts_available:
                    ...
                    close_shift = Utils.check_close_sfift(self.priority_queues[tickets_info[ticket_id]["team"]], tickets_info, self.aux_data)
                    ...
                    if "analysed_in_shift" not in tickets_info[ticket_id]:
                        ...
                        Utils.debug_and_log_data(self.aux_data.debug, self.aux_data.logger, f'id {tickets_info[ticket_id]["id"]} analysed in shift {curr_shift}')
            else:   
    ...
    def update_ticket_transfer_ticket(self, ticket, family_subtechniques):
        ...
        if ticket["replication_status"] != None:
            ...
            Utils.debug_and_log_data(self.aux_data.debug, self.aux_data.logger, f'Prev escalated: {ticket["action"]}')
            ...
            Utils.debug_and_log_data(self.aux_data.debug, self.aux_data.logger, f'Action escalated: {action}')

            if Utils.check_shift_ending(ticket_date, action_dur, self.aux_data):
        ...
        Utils.debug_and_log_data(self.aux_data.debug, self.aux_data.logger, f'Allocated: {ticket_date}, Fixed: {ticket["fixed"]}')
    ...
    def check_next_analyst_action(self, ticket, analyst, subfamily_actions, special_tech, use_subfamily_action_choices, use_same_action_choices):
        ...
        if subfamily in subfamily_actions.keys() and team in subfamily_actions[subfamily].keys() and analyst in subfamily_actions[subfamily][team].keys():
            ...
            if same_action:
                ...
                Utils.debug_and_log_data(self.aux_data.debug, self.aux_data.logger, f'Gen: {analyst} is going to use the prev action {analyst_sol}')
                ...
                Utils.debug_and_log_data(self.aux_data.debug, self.aux_data.logger, f'Gen: {analyst} is performing a new action: {analyst_sol}')
                actions_status = f'{analyst} is using a new action'
        else:
            Utils.debug_and_log_data(self.aux_data.debug, self.aux_data.logger, "Gen: Subfamily and team member don't exists!")
            ...
            if use_subfamily_action:
                ...
                Utils.debug_and_log_data(self.aux_data.debug, self.aux_data.logger, f'Gen: {analyst} is going to use the subfamily action: {analyst_sol}')
                actions_status = f'{analyst} is using the subfamily action'
            # Creates a new action based on the family
            else:
                analyst_sol = Utils.build_analyst_action(family, subfamily, team, analyst, self.subfamily_pool[subfamily]["teams_actions"][team], self.family_steps_pool[team], special_tech, self.aux_data)
                Utils.debug_and_log_data(self.aux_data.debug, self.aux_data.logger, f'Gen: {analyst} is going to use a new action: {analyst_sol}')
    ...
    def check_valid_analyst(self, all_tickets, ticket_date, subfamily, team, member, action_duration, outlier):
        ...
        if not Utils.check_shift_ending(ticket_date, action_duration, self.aux_data):
            user_status= True
            Utils.debug_and_log_data(self.aux_data.debug, self.aux_data.logger, f'Curr Date: {ticket_date} {member} - action dur - {action_duration}')
        else:
            user_status= False
            Utils.debug_and_log_data(self.aux_data.debug, self.aux_data.logger, f'Curr Date: {ticket_date} {member} - surpassing his shift - {action_duration}')
    ...
    def find_next_fastest_analyst(self, ticket, analysts_available):
        ...
        if len(free_users) == 1:
            ...
            if self.analysts_info[team]["analysts"][user]["queue"]:
                Utils.debug_and_log_data(self.aux_data.debug, self.aux_data.logger, f'{user} is occupied until {self.analysts_info[team]["analysts"][user]["queue"][-1]}')
                user = None
            ...
            if subfamily_steps_speeds[subfamily][team][analyst][step]["last_incident"] != -1 and step not in action:
                ...
                if time_diff > 10080:  # 1 week
                    ...
                    if subfamily_steps_speeds[subfamily][team][analyst][step]["speed"] < 1.98:
                        Utils.debug_and_log_data(self.aux_data.debug, self.aux_data.logger, f'Lose skill of {analyst} on step {step}')
